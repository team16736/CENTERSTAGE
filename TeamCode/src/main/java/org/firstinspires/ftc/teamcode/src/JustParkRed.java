...
public class JustParkRed extends HelperActions {
    ...
    public void runOpMode() {
        ...
        if (opModeIsActive()) {
            ...
            gyroActions.initEncoderGyroStrafeStateMachine(speed, 23, isBlue);
            while (gyroActions.encoderGyroStrafeStateMachine(speed, 23, 0, isBlue)){
                liftyUppyActions.update();
                if (liftyUppyActions.flippyTurny.getCurrentPosition() > 300) {
                    liftyUppyActions.goToPreset(false, true, false, false);
                }
            ...
            gyroActions.initEncoderGyroStrafeStateMachine(speed, 18, isBlue);
            while (gyroActions.encoderGyroStrafeStateMachine(speed, 18, 0, isBlue)){
                liftyUppyActions.update();
            }
            placerActions.releasePixel();
            sleep(800);
